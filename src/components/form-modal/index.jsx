...
import { addNewComment, editComment, editFormComment, openFormComment, updateStateFormComment } from '../../state/comment-slice';
import { addNewPost, editFormPost, editPost, openFormPost, updateStateFormPost } from '../../state/post-slice';

function FormModal({type}) {
  const {isOpenFormPost, isEditFormPost, stateFormPost} = useSelector((state) => state.post);
  const {isOpenFormComment, isEditFormComment, stateFormComment} = useSelector((state) => state.comment);
            ...
            <form onSubmit={handleFormPost}>
              <div className="mb-4">
                <input
                  ...
                  onChange={(e) => dispatch(updateStateFormPost({id: stateFormPost.id,title: e.target.value, body: stateFormPost.body}))}
                />
              </div>
              <div className="mb-4">
                <input
                  ...
                  onChange={(e) => dispatch(updateStateFormPost({id: stateFormPost.id, title: stateFormPost.title, body: e.target.value}))}
                />
              </div>
              <button 
            ...
            <form onSubmit={handleFormComment}>
              <div className="mb-4">
                <input
                  ...
                  onChange={(e) => dispatch(updateStateFormComment({id: stateFormComment.id, email: e.target.value, name: stateFormComment.name, body: stateFormComment.body}))}
                />
                <input
                  className="mb-3 shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  id="Name"
                  ...
                  onChange={(e) => dispatch(updateStateFormComment({id: stateFormComment.id, email: stateFormComment.email, name: e.target.value, body: stateFormComment.body}))}
                />
                <input
                  className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  id="Comment"
                  ...
                  onChange={(e) => dispatch(updateStateFormComment({id: stateFormComment.id, email: stateFormComment.email, name: stateFormComment.name, body: e.target.value}))}
                />
              </div>
              <button 
                className="w-full bg-blue-500 p-3 text-white"
